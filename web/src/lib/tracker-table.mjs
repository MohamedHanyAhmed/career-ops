 ...
 * @returns {{n: string, date: string, company: string, via: string, role: string, score: string, status: string, pdf: string, report: string, notes: string, location: string, url: string}[]}
 */
  ...
  for (const raw of lines) {
    ...
    if (map) {
      ...
      rows.push({
        ...
        notes: at("notes"), location: at("location"), url: at("url"),
      });
    } else {
      ...
      rows.push({ n, date, company, via: "", role, score, status, pdf, report, notes: rest.join(" | "), location: "", url: "" });
    }
  }
  return rows;
}
